...
#1.0 surface area calculations based on parallelopiped/rectangular prism area
  ...
  subSeal <- which(dShell$seal == 'Y' &  dShell$taxon == 'Saccostrea')
  dShell[subSeal,'finalSA'] <- 2*(dShell[subSeal,'xDim'] * dShell[subSeal,'yDim'])
  subSealPect <- which(dShell$seal == 'Y' &  dShell$taxon == 'Pecten')
  dShell[subSealPect,'finalSA'] <- 2*(dShell[subSealPect,'calcX'] * dShell[subSealPect,'yDim'])
  subSealAna <- which(dShell$seal == 'Y' &  dShell$taxon == 'Anadara')
...
#3.0 calculation of porosity using calcite and aragonite
